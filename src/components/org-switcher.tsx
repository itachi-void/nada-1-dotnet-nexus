...
export function OrgSwitcher() {
  ...
  const activeMembership = MOCK_ORGANIZATIONS.find(
    (m) => m.organization.id === activeOrgId
  ) || MOCK_ORGANIZATIONS[0];

  const displayOrganization = activeMembership.organization;
      ...
      <SidebarMenuItem>
        <DropdownMenu>
            ...
            >
              ...
              >
                ...
                <span className='text-muted-foreground truncate text-xs'>
                  {activeMembership.role || 'Organization'}
                </span>
              </div>
          ...
          >
            ...
            {MOCK_ORGANIZATIONS.map((membership, index) => {
              const isActive = membership.organization.id === activeOrgId;
              return (
                <DropdownMenuItem
                  key={membership.id}
                  onClick={() => setActiveOrgId(membership.organization.id)}
                  className='gap-2 p-2'
                >
